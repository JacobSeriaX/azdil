...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (newOrderForm) {
        newOrderForm.onsubmit = function(event) {
            ...
            database.ref().update(updates)
                .then(() => {
                    ...
                    closeFormModal();
                    newOrderForm.reset(); // Очистка формы
                })
                .catch(error => {
        ...
        console.error("Форма нового заказа не найдена.");
    }
    ...
    function renderOrders(snapshot) {
        ...
        if (!waitingContainer || !completedContainer) {
            console.error("Контейнеры для заказов не найдены.");
            return;
        }
    ...
    function showOrderInfo(order) {
        ...
        if (!modal) {
            console.error("Модальное окно информации о заказе не найдено.");
            return;
        }
    ...
    window.deleteOrder = function(orderKey) { // Экспонирование функции в глобальную область
    ...
    window.updateOrderStatus = function(orderKey, newStatus) { // Экспонирование функции в глобальную область
    ...
    function closeFormModal() {
        ...
        if (modal) {
            ...
            console.error("Модальное окно оформления заказа не найдено.");
        }
    }
    ...
    function filterOrders() {
        ...
        ordersContainers.forEach(orderElement => {
            ...
            if (!statusElement) {
                console.error("Элемент статуса заказа не найден.");
                return;
            }
